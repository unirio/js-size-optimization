...
res.append = function append(field, val) {
    ...
    if (prev) {
        ...
        value = Array.isArray(prev) ? prev.concat(val) : Array.isArray(val) ? [prev].concat() : [
            prev,
            val
        ];
    }
...
res.clearCookie = function clearCookie(name, options) {
    var opts = merge({ expires: new Date(1) }, options);
...
function sendfile(res, file, options, callback) {
    ...
    if (options.headers) {
        ...
        file.on('headers', function headers() {
