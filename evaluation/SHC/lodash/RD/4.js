...
(function () {
   ...
   */
     ...
     */
        ...
        var ArrayBuffer = context.ArrayBuffer, Reflect = context.Reflect, Set = getNative(context, 'Set'), Uint8Array = context.Uint8Array, WeakMap = getNative(context, 'WeakMap'), clearTimeout = context.clearTimeout, enumerate = Reflect ? Reflect.enumerate : undefined, getPrototypeOf = Object.getPrototypeOf, parseFloat = context.parseFloat, pow = Math.pow, propertyIsEnumerable = objectProto.propertyIsEnumerable, splice = arrayProto.splice;
        /* Native method references for those with the same name as other `lodash` methods. */
        var nativeCeil = Math.ceil, nativeCreate, nativeFloor = Math.floor, nativeIsFinite = context.isFinite, nativeKeys = Object.keys, nativeMax = Math.max, nativeMin = Math.min, nativeParseInt = context.parseInt, nativeRandom = Math.random;
     ...
     */
        function baseDifference(array, values) {
            ...
            var index = -1, indexOf = getIndexOf(), isCommon, cache, valuesLength = values.length;
            if (cache) {
     ...
     */
        ...
        if (enumerate && !propertyIsEnumerable.call({ 'valueOf': 1 }, 'valueOf')) {
            baseKeysIn = function () {
                var data, iterator = enumerate(object);
     ...
     */
        function createCtorWrapper(Ctor) {
            return function () {
                ...
                case 5:
                    return new Ctor(args[0], args[1], args[2], args[4]);
                case 6:
     ...
     */
        function isNative(value) {
            ...
            return isObjectLike(value) && (isHostObject(value) ? reIsNative : reIsHostCtor).test();
        }
        /**
     ...
     */
        function keysIn(object) {
            ...
            while (++index < propsLength) {
                var key = props[index];
                if (!(skipIndexes && isIndex(key)) && !(key == 'constructor' && (isProto || !hasOwnProperty.call(object, key)))) {
                    result.push(key);
                }
            }
            return result;
