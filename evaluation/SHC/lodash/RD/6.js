...
(function () {
   ...
   */
    var reRegExpChars = /[\\^$.*+?()[\]{}|]/g, reHasRegExpChars = RegExp();
   ...
   */
    function runInContext(context) {
        ...
        context = context ? _.defaults(root.Object(), _.pick(root, contextProps)) : root;
        /** Native constructor references. */
        var Array = context.Array, Date = context.Date, Error = context.Error, Function = context.Function, Math = context.Math, Number = context.Number, Object = context.Object, RegExp = context.RegExp, TypeError = context.TypeError;
     ...
     */
        ...
        var ArrayBuffer = context.ArrayBuffer, Reflect = context.Reflect, Set = getNative(), Uint8Array = context.Uint8Array, WeakMap = getNative(context, 'WeakMap'), clearTimeout = context.clearTimeout, enumerate = Reflect ? Reflect.enumerate : undefined, getPrototypeOf = Object.getPrototypeOf, pow = Math.pow, setTimeout = context.setTimeout, splice = arrayProto.splice;
     ...
     */
        function baseDifference(array, values) {
            ...
            var index = -1, indexOf = getIndexOf(), isCommon = indexOf === baseIndexOf, cache = isCommon && values.length >= LARGE_ARRAY_SIZE ? createCache() : null, valuesLength = values.length;
            outer:
                while (++index < length) {
                    var value = array[index];
        ...
        function baseIsEqualDeep(object, other, equalFunc, customizer, isLoose, stackA, stackB) {
            ...
            if (!isLoose) {
                ...
                if (objIsWrapped || othIsWrapped) {
                    return equalFunc(objIsWrapped ? object.value() : object, othIsWrapped ? other.value() : other, customizer, isLoose);
                }
            }
            if (!isSameTag) {
                return false;
     ...
     */
        function createCtorWrapper(Ctor) {
            return function () {
                ...
                case 6:
                    return new Ctor(args[0], args[1], args[2], args[4], args[5]);
                case 7:
                    return new Ctor(args[0], args[1], args[2], args[3], args[4], args[5]);
                }
                var thisBinding, result = Ctor.apply(thisBinding);
     ...
     */
        function mergeData(data, source) {
            ...
            if (value) {
                ...
                data[6] = partials ? replaceHolders(data[5]) : copyArray(source[6]);
            }
     ...
     */
        function includes(collection, target, fromIndex, guard) {
            ...
            return typeof collection == 'string' || !isArray() && isString(collection) ? fromIndex <= length && collection.indexOf(target, fromIndex) > -1 : !!length && getIndexOf(collection, target, fromIndex) > -1;
        }
        /**
     * Creates an object composed of keys generated from the results of running
     * each element of `collection` through `iteratee`. The corresponding value
     ...
     */
        function debounce(func, wait, options) {
            var args, maxTimeoutId, result, stamp, timeoutId, trailingCall, lastCalled = 0, leading = false, maxWait, trailing = true;
            if (typeof func != 'function') {
                throw new TypeError(FUNC_ERROR_TEXT);
            }
