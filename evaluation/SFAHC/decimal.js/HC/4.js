...
(function (globalScope) {
   ...
   */
    P.hyperbolicTangent = P.tanh = function () {
        ...
        Ctor.precision = pr + 7;
        Ctor.rounding = 1;
        return divide(x.sinh(), x.cosh(), Ctor.precision = pr);
    };
    /*
   ...
   */
    P.inverseCosine = P.acos = function () {
        var halfPi, x = this, Ctor = x.constructor, k, pr = Ctor.precision, rm = Ctor.rounding;
        if (k !== -1) {
            return k === 0    // |x| is 1
 ? x.isNeg() ? getPi(Ctor, pr) : new Ctor(0)    // |x| > 1 or x is NaN
 : new Ctor(NaN);
        }
        if (x.isZero())
   ...
   */
    P.inverseSine = P.asin = function () {
        ...
        if (k !== -1) {
            // |x| is 1
            if (k === 0) {
                halfPi = getPi(Ctor, rm).times(0.5);
                halfPi.s = x.s;
                return halfPi;
            }
            // |x| > 1 or x is NaN
   ...
   */
    P.minus = P.sub = function (y) {
        ...
        if (k) {
            ...
            i = Math.max(Math.ceil(pr / LOG_BASE)) + 2;
            if (k > i) {
                k = i;
                d.length = 1;
            }
   ...
   */
    P.plus = P.add = function (y) {
        ...
        y.d = xd;
        y.e = getBase10Exponent(xd, e);
        return external ? finalise(y, rm) : y;
    };
    /*
   ...
   */
    P.squareRoot = P.sqrt = function () {
        ...
        for (;;) {
            t = r;
            r = t.plus(divide(x, t, sd + 2)).times(0.5);
            // TODO? Replace with for-loop and checkRoundingDigits.
            if (digitsToString(t.d).slice(0, sd) === (n = digitsToString(r.d)).slice(0, sd)) {
                n = n.slice(sd - 3, sd + 1);
 ...
 : !xd || !yd ? y.s / 0 : y.s * 0);
   ...
   */
    P.toOctal = function (sd, rm) {
        return toStringBinary(this, 8, sd);
    };
    /*
   ...
   */
    ...
    function getPi(Ctor, sd, rm) {
        ...
        return finalise(new Ctor(PI), sd, rm);
    }
   ...
   */
    function naturalExponential(x, sd) {
        var denominator, guard, j, pow, sum, t, wpr, rep = 0, i = 0, k = 0, Ctor = x.constructor, rm, pr = Ctor.precision;
        // 0/NaN/Infinity?
        if (!x.d || !x.d[0] || x.e > 17) {
   ...
   */
    function naturalLogarithm(y, sd) {
        ...
        // where x = (y - 1)/(y + 1)    (|x| < 1)
        sum = numerator = x = divide(x.minus(1), x.plus(1), 1);
        x2 = finalise(x.times(x), 1);
        denominator = 3;
        for (;;) {
            numerator = finalise(numerator.times(x2), wpr, 1);
            t = sum.plus(divide(numerator, new Ctor(denominator), 1));
            if (digitsToString(t.d).slice(0, wpr) === digitsToString(sum.d).slice(0, wpr)) {
                sum = sum.times(2);
   ...
   */
    ...
    function toLessThanHalfPi(Ctor, x) {
        var t, isNeg = x.s < 0, pi = getPi(Ctor, Ctor.precision), halfPi = pi.times(0.5);
        x = x.abs();
        if (x.lte(halfPi)) {
            quadrant = isNeg ? 4 : 1;
            return x;
   ...
   */
    function toStringBinary(x, baseOut, sd, rm) {
        var base, e, i, k, len, roundUp, str, xd, y, Ctor = x.constructor, isExp;
        if (isExp) {
