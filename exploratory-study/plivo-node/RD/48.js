...
var request = function (action, method, params, callback, optional) {
    ...
    var auth = 'Basic ' + new Buffer(plivo.options.authId + ':' + plivo.options.authToken).toString('base64');
    var headers = {
        'Authorization': auth,
    ...
    if (method == 'POST') {
        ...
        var body = JSON.stringify(params);
        request_options.json = true;
        request_options.body = body, Request.put(request_options, function (error, response, body) {
            callback(response.statusCode, body);
        });
    }
};
...
plivo.create_signature = function (url, params) {
    ...
    var signature = crypto.createHmac('sha1', plivo.options.authToken).update(new Buffer(toSign, 'utf-8')).digest('base64');
    return signature;
};
...
plivo.hangup_conference = function (params) {
    var action = 'Conference/' + params['conference_id'] + '/';
    delete params.conference_id;
    var method = 'DELETE';
...
plivo.speak_conference_member = function (params) {
