...
function parse(source) {
    ...
    // global scope
    var currentScope = scopeManager.acquire();
    var calls = [];
...
function findModule(callExpression, scope) {
    var module = {};
    var calleeObj = _.get(callExpression, 'callee.object', {}), calleeProp = _.get(callExpression, 'callee.property', {});
    if (calleeObj.type === 'CallExpression') {
        ...
        if (calleeProp.name === 'module' && calleeObj.name === 'angular') {
            module.name = _.get(callExpression, 'arguments[0].value');    // or module can be stored in variable; find this variable then
...
function findProviderDeps(depsFn) {
    ...
    var depsFnBodyExpressions = _.get(depsFnBody);
    return deps;
}
...
function extractVariableDeps(varName) {
    var varNode = findVariable(varName, scope);
    if (!varNode) {
    }
    var params = [];
    var varType = _.get(varNode);
